...
class UserSearchViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        initiateUserSearchViewModel()
    }
        
    func initiateUserSearchViewModel(){
        self.userSearchViewModel =  UserSearchViewModel()
    }
    ...
    func fectchUserDetailsFromGitHub(text: String) {
        if ReachabilityManager.shared.isReachable() == true {
            ...
            self.userSearchViewModel.fectchUserDetailsFromGitHub(text: text) { [weak self] (error, user) in
                guard let strongSelf = self else {
                    return
                }
                if error == nil {
